...
# --- YEH NAYA ADVANCED F-SUB FUNCTION HAI ---
async def check_fsub_status(bot, user_id, channel_id):
    """
    User ka F-Sub status check karta hai.
    Returns: "MEMBER", "PENDING", ya "NEW"
    """
    try:
        ...
        # User member nahi hai. Ab pending check karo.
        pass
    except Exception as e:
        logger.error(f"Error checking chat member: {e}")
        # Agar koi error aaye, to safety ke liye 'NEW' maano
        return "NEW"
    ...
    # Case 3: User ne kuch nahi kiya (New)
    return "NEW"


async def is_req_subscribed(bot, query):
    # This function is now part of the legacy check, 
    # but the new `check_fsub_status` is more comprehensive.
    # We keep it as is, but commands.py will use the new function.
...
def list_to_str(k):
    if not k:
        ...
        # --- FIX: Correctly joins elements with a comma and space ---
