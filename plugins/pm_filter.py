...
# --- FIX: 'is_req_subscribed' ko yahan se hata diya gaya hai ---
...
async def pm_search(client, message):
    if IS_PM_SEARCH:
        # <--- FIX: Removed redundant 'if/else' block.
        # The previous 'if' for languages was useless because
        # 'auto_filter' was called in both the 'if' and 'else' branches.
...
async def group_search(client, message):
    ...
    if settings["auto_filter"]:
        ...
        # This logic is correct (not redundant) because the 'return' exits the function.
...
async def next_page(bot, query):
    ...
    if settings['tutorial'] and settings['tutorial'].startswith("http"):
        btn.append(
    ...
    if settings["link"]:
        # The commented-out code here was correctly identified as redundant.
...
async def lang_search(client: Client, query: CallbackQuery):
    ...
    # Note: This logic is inefficient. It filters results *after* getting them from the DB,
    # which can lead to incorrect pagination (e.g., empty pages).
    # A better solution would be to have the DB handle the language filtering.
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    # --- FIX: 'checksub' wala poora block yahan se delete kar diya gaya hai ---
    # Yeh logic ab 'Commands.py' mein /start handler ke zariye manage hota hai.
...
async def auto_filter(client, msg, spoll=False):
    ...
    if offset != "":
        ...
        # <--- FIX: Removed 7 lines of redundant/dead code here.
        # The code block below was re-defining 'key', 'BUTTONS[key]', and 'req'
        # and then performed a 'try/except' on 'offset' that was never used.
...
async def advantage_spell_chok(message):
    ...
    try:
        # <--- FIX: Using the cleaned query 'RQST' instead of the original 'search'
        movies = await get_poster(RQST, bulk=True)
    except:
        ...
        return
    if not movies:
        # <--- FIX: Using the cleaned query 'RQST' for the Google link
        google = RQST.replace(" ", "+")
        button = [[
            InlineKeyboardButton("🔍 ᴄʜᴇᴄᴋ sᴘᴇʟʟɪɴɢ ᴏɴ ɢᴏᴏɢʟᴇ 🔍", url=f"https://www.google.com/search?q={google}")
        ]]
        # <--- FIX: Using the cleaned query 'RQST' in the error message
        k = await message.reply_text(text=script.I_CUDNT.format(RQST), reply_markup=InlineKeyboardMarkup(button))
