...
# --- FIX 1: IMPORTED check_fsub_status ---
from utils import (
    get_settings, save_group_settings, is_req_subscribed, get_size, 
...
async def start(client:Client, message): 
    ...
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        # --- FIX 2: Added message.from_user.id to get_status() ---
    ...
    if len(message.command) != 2:
        ...
        await message.reply_text(script.START_TXT.format(message.from_user.mention, get_status(message.from_user.id), message.from_user.id),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        return

    # --- FIX 3: REPLACED OLD F-SUB LOGIC WITH NEW 3-CASE LOGIC ---
    fsub_status = "MEMBER" # Default agar AUTH_CHANNEL off hai
    if AUTH_CHANNEL:
        fsub_status = await check_fsub_status(client, message.from_user.id, int(AUTH_CHANNEL))
        ...
        if fsub_status == "NEW":
            ...
            btn = [[
                InlineKeyboardButton("REQUEST TO JOIN ⛔️", url=invite_link.invite_link)
            ]]
            ...
            if message.command[1] != "subscribe":
                try:
                    ...
                    btn.append(
                        [InlineKeyboardButton("♻️ ᴛʀʏ ᴀɢᴀɪN ♻️", url=f"https://t.me/{temp.U_NAME}?start={message.command[1]}")]
                    )

            await client.send_message(
                chat_id=message.from_user.id,
                text=script.FSUB_REQUEST_MSG, # Case 3 ka naya message
                reply_markup=InlineKeyboardMarkup(btn),
                parse_mode=enums.ParseMode.HTML
            )
            return
    # --- NAYA LOGIC YAHAN KHATAM HOTA HAI ---
    # Case 1 (Member) aur Case 2 (Pending) yahan continue karenge
    ...
    if len(message.command) == 2 and message.command[1] in ["subscribe", "error", "okay", "help", "buy_premium"]:
        ...
        await message.reply_text(
            text=script.START_TXT.format(message.from_user.mention, get_status(message.from_user.id), message.from_user.id),
            reply_markup=reply_markup,
            parse_mode=enums.ParseMode.HTML
        )
        return
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            d = await m.reply_text(
                text=msg.format(message.from_user.mention, get_status(message.from_user.id)),
    ...
    if data and data.startswith("allfiles"):
        ...
        for file in files:
            ...
            # --- FIX 4: CASE 2 (PENDING) KA MESSAGE ADD KARNA ---
            if fsub_status == "PENDING":
                f_caption = script.FSUB_PENDING_MSG + "\n\n" + f_caption
            # --- YAHAN TAK ---
