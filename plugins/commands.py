...
# <--- FIX: Added MediaEmpty to handle broken files
from pyrogram.errors import ChatAdminRequired, FloodWait, UserIsBlocked, PeerIdInvalid, MediaEmpty
...
# <--- FIX: Added a helper function for non-blocking auto-delete
...
async def start(client:Client, message): 
    ...
    if data and data.startswith("allfiles"):
        ...
        # <--- FIX: Get settings once before the loop
        ...
        for file in files:
            ...
            # <--- FIX: Added non-blocking auto-delete for batch files
            if settings.get("auto_delete"):
                asyncio.create_task(auto_delete_message(d, DELETE_TIME))
        return
    ...
    # This block replaces your original send_cached_media call
    try:
        d = await client.send_cached_media(
            chat_id=message.from_user.id,
            file_id=file_id,
        ...
        # Use the non-blocking auto-delete logic
        if settings.get("auto_delete"):
            asyncio.create_task(auto_delete_message(d, DELETE_TIME))

    except MediaEmpty:
        # 1. Inform the user
        await message.reply_text("<b>⚠️ Sorry, this file seems to be broken or deleted.\n\nI am removing it from my database. Please try searching again.</b>")
        
        # 2. Log and delete the bad file from MongoDB
        logger.error(f"MediaEmpty error for file_id: {file_id}. Deleting from DB.")
        try:
            await Media.collection.delete_one({'_id': file_id})
        except Exception as e:
            logger.error(f"Failed to delete bad file_id {file_id} from DB: {e}")
            
    except Exception as e:
        # Catch any other sending errors
        logger.error(f"Error sending file {file_id}: {e}")
        await message.reply_text("<b>⚠️ An unknown error occurred while trying to send this file.</b>")
...
async def set_time(client, message):
    ...
    chat_type = message.chat.type
    if chat_type not in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        return await message.reply_text("<b>ᴜsᴇ Tʜɪs ᴄᴏᴍᴍᴀɴᴅ ɪɴ ɢʀᴏᴜᴘ...</b>")       
    grp_id = message.chat.id
    title = message.chat.title
    if not await is_check_admin(client, grp_id, message.from_user.id):
        return await message.reply_text('<b>ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ɪɴ ᴛʜɪꜱ ɢʀᴏᴜᴘ</b>')
